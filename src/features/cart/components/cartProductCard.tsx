...
export default function CartProductCard({ info }: { info: CartItem }) {
	...
	return (
		<div className="space-y-4">
			{/* Desktop Card */}
			<article className="hidden sm:flex bg-[#e6e6e6] rounded-2xl p-5 flex-col lg:flex-row gap-5
				shadow-[6px_6px_12px_#c5c5c5,-6px_-6px_12px_#ffffff]">
				{/* Left: Image & Stock Badge */}
				<div className="flex flex-col items-center gap-3 shrink-0">
					<Link href={`/products/${id}`}>
						<div className="relative h-28 w-28 rounded-xl bg-[#e6e6e6] overflow-hidden
					...
					<span
						className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-[#e6e6e6]
							shadow-[2px_2px_4px_#c5c5c5,-2px_-2px_4px_#ffffff]
							${quantity > 0 ? "text-emerald-600" : "text-rose-600"}`}
					>
						{quantity > 0 && (
				...
				<div className="flex-1 space-y-3">
					<Link href={`/products/${id}`}>
						<h3 className="text-lg font-semibold text-gray-800 leading-snug hover:text-emerald-600 transition-colors">
							{title}
						</h3>
					</Link>
					<div className="flex flex-wrap items-center gap-2">
						<span className="px-3 py-1 rounded-full text-xs font-medium text-emerald-700 bg-[#e6e6e6]
							shadow-[2px_2px_4px_#c5c5c5,-2px_-2px_4px_#ffffff]">
					...
					{/* Quantity Selector */}
					<div className="flex items-center gap-1 pt-1 bg-[#e6e6e6] w-fit p-1 rounded-lg
						shadow-[inset_2px_2px_4px_#c5c5c5,inset_-2px_-2px_4px_#ffffff]">
						<button
							disabled={count <= 1}
							onClick={() => handleUpdateQuantity(count - 1)}
							type="button"
							className="h-8 w-8 flex items-center justify-center rounded-lg bg-[#e6e6e6] text-gray-500 transition-all disabled:opacity-50 disabled:cursor-not-allowed
								shadow-[2px_2px_4px_#c5c5c5,-2px_-2px_4px_#ffffff]"
						...
						<button
							...
							onClick={() => handleUpdateQuantity(count + 1)}
							type="button"
							className="h-8 w-8 flex items-center justify-center rounded-lg bg-[#e6e6e6] text-emerald-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed
								shadow-[2px_2px_4px_#c5c5c5,-2px_-2px_4px_#ffffff]"
				...
				<div className="flex items-end gap-4 lg:ml-auto">
					...
					<button
						...
						type="button"
						className="h-11 w-11 rounded-xl flex items-center justify-center bg-[#e6e6e6] text-rose-500 transition-all
			...
			{/* Mobile Card */}
			<article className="sm:hidden bg-[#e6e6e6] rounded-xl p-4
				shadow-[6px_6px_12px_#c5c5c5,-6px_-6px_12px_#ffffff]">
					...
					<div className="flex-1 min-w-0">
						<div className="flex items-start justify-between gap-2">
							<div className="min-w-0">
								<Link href={`/products/${id}`}>
									<h3 className="text-sm font-semibold text-gray-800 leading-snug line-clamp-2 hover:text-emerald-600 transition-colors">
										{title}
									</h3>
								</Link>
							...
							<button
								onClick={handleRemove}
								type="button"
								className="p-2 rounded-lg text-gray-400 hover:text-rose-500 bg-[#e6e6e6] transition-all shrink-0
									shadow-[2px_2px_4px_#c5c5c5,-2px_-2px_4px_#ffffff]"
								aria-label="Remove item"
							>
						...
						{/* Stock Badge - Inline */}
						<span
							className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium mt-1 bg-[#e6e6e6]
								shadow-[inset_1.5px_1.5px_3px_#c5c5c5,inset_-1.5px_-1.5px_3px_#ffffff]
								${quantity > 0 ? "text-emerald-600" : "text-rose-600"}`}
						>
							{quantity > 0 && (
				...
				<div className="flex items-center justify-between mt-4 pt-3">
					{/* Quantity Selector */}
					<div className="flex items-center gap-1 bg-[#e6e6e6] p-1 rounded-lg
						shadow-[inset_2px_2px_4px_#c5c5c5,inset_-2px_-2px_4px_#ffffff]">
						<button
							disabled={count <= 1}
							onClick={() => handleUpdateQuantity(count - 1)}
						...
						<span className="w-8 text-center text-gray-800 font-semibold text-sm">
							{count}
						</span>
						<button
							disabled={count >= quantity}
